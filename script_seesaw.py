...
vec_delta_ideal4 = np.zeros(
    (num_sets, num_exps_per_set)
)  # vector of final deltas (4 random input states, only shot noise)
vec_delta_noisy4st001 = np.zeros(
    (num_sets, num_exps_per_set)
)  # vector of final deltas (4 random input states, coherent noise)
...
for jj in range(num_sets):
    ...
    for kk in range(num_exps_per_set):
        ...
        noisy_freq4001 = generate_frequencies_noisy(
            num_shots, M, rhoRand4, 0.0, 0.01
        )  # coherent noise with epsilon = 0.01

        vec_delta_ideal4[jj, kk] = run_seesaw(rhoRand4, id_freq4, nu_delta)[
            2
        ]  # delta at the final seesaw step
        vec_delta_noisy4st001[jj, kk] = run_seesaw(rhoRand4, noisy_freq4001, nu_delta)[
            2
        ]  # delta at the final seesaw step
