...
def amp_damping_channel(
    rho, gamma
):  # create an amplitude damping channel with noise strength p
...
def phase_damping_channel(
    rho, gamma
):  # create a phase damping channel with noise strength p
    K0 = np.array([[1, 0], [0, np.sqrt(1 - gamma)]])
...
def generate_frequencies(num_shots, list_effects, list_states):
    ...
    num_outcomes = len(
        list_effects
    )  # number of possible outcomes (i.e., number of effects)
    ...
    for kk in range(0, num_inputStates):
        ...
        sample = np.random.choice(
            range(num_outcomes), size=num_shots, p=prob_matrix[:, kk]
        )
...
def generate_frequencies_noisy(num_shots, list_effects, list_states, p, rotMagnitude):
    ...
    for kk in range(0, num_inputStates):
        ...
        for jj in range(0, num_outcomes):
            # We compute the ideal probabilities according to Born's rule
            prob_matrix[jj, kk] = np.trace(
                list_effects[jj]
                @ randRot
                @ random_noise(list_states[kk], p)
                @ randRot.conj().transpose()
            )
    ...
    for kk in range(0, num_inputStates):
        # We sample the different experimental outcomes (from module "random")
        sample[:, kk] = np.array(
            random.choices(range(num_outcomes), weights=prob_matrix[:, kk], k=num_shots)
        )
...
def generate_frequencies_noisy_states_vec(
    num_shots, list_effects, list_states, p_vec, rotMagnitude_vec
):
    """Function to generate the experimental frequencies for the different effects (in list_effects) of a POVM obtained
    ...
    for kk in range(0, num_inputStates):
        ...
        list_states_noisy.append(
            randRot
            @ random_noise(list_states[kk], p_vec[kk])
            @ randRot.conj().transpose()
        )
        for jj in range(0, num_outcomes):
            # We compute the ideal probabilities according to Born's rule
...
def gen_noisy_states(rhoInput_list, p, rotMagnitude):
    ...
    for kk in range(len(rhoInput_list)):
        randRot = random_rotation(dim, rotMagnitude, rotMagnitude)
        list_states_noisy.append(
            randRot @ random_noise(rhoInput_list[kk], p) @ randRot.conj().transpose()
        )
...
def run_SDP_QMT(list_states, frequencies, SDPtype="norm", **kwargs):
    ...
    constraints_list = [
        np.sum([Pi_list[jj] for jj in range(num_outcomes)]) == np.eye(dim)
    ]
    ...
    if SDPtype == "singleDelta":
        ...
        for jj in range(0, num_outcomes):
            ...
            for kk in range(0, num_inputStates):
                constraints = [
                    ...
                    cp.real(cp.trace(Pi_list[jj] @ list_states[kk]))
                    <= (frequencies[jj, kk] + delta),
                    cp.real(cp.trace(Pi_list[jj] @ list_states[kk]))
                    >= (frequencies[jj, kk] - delta),
                ]
        ...
        deltamatrix = cp.Variable(
            (num_outcomes, num_inputStates)
        )  # matrix of deltas we minimized
        ...
        for jj in range(0, num_outcomes):
            ...
            for kk in range(0, num_inputStates):
                constraints = [
                    ...
                    cp.real(cp.trace(Pi_list[jj] @ list_states[kk]))
                    <= (frequencies[jj, kk] + deltamatrix[jj, kk]),
                    cp.real(cp.trace(Pi_list[jj] @ list_states[kk]))
                    >= (frequencies[jj, kk] - deltamatrix[jj, kk]),
                ]
                constraints_list += constraints
        ...
        for jj in range(0, num_outcomes):
            ...
            for kk in range(0, num_inputStates):
                norm += cp.abs(
                    cp.trace(Pi_list[jj] @ list_states[kk]) - frequencies[jj, kk]
                )
    ...
    if SDPtype == "singleDelta":
        ...
        for kk in range(0, num_inputStates):
            ...
            for jj in range(0, num_outcomes):
                constraints = [
                    # inequalities for the SDP
                    cp.real(cp.trace(rho_list[kk] @ list_effects[jj]))
                    <= (frequencies[jj, kk] + delta),
                    cp.real(cp.trace(rho_list[kk] @ list_effects[jj]))
                    >= (frequencies[jj, kk] - delta),
                ]
        ...
        for jj in range(0, num_outcomes):
            for kk in range(0, num_inputStates):
                norm += cp.abs(
                    cp.trace(rho_list[kk] @ list_effects[jj]) - frequencies[jj, kk]
                )
        obj = cp.Minimize(norm)
...
def gen_frequencies_list_noisy(
    num_shots, list_effects, list_states, p, rotMagnitude, num_k=10
):
    ...
    return_list = []
    list_states_noisy = gen_noisy_states(
        rhoInput_list=list_states, p=p, rotMagnitude=rotMagnitude
    )
...
def run_seesaw_crossV(
    ...
    num_k = len(
        frequencies_list
    )  # number of subsets into which we devide the total dataset

    av_exp_frequencies = np.mean(
        frequencies_list, axis=0
    )  # experimental frequencies for the whole subset
    list_distance = (
        []
    )  # distances between A and B experimental frequencies for each partition

    for jj in range(
        num_repetitions
    ):  # loop over the different repetitions (different partitions in each repetition)

        random_list = random.sample(
            range(num_k), num_sampling
        )  # we sample num_sampling subsets over the total number of subsets

        # we compute the average experimental frequencies for the sampled ("A") subset
        matrix_mean_training = np.mean(
            [x for i, x in enumerate(frequencies_list) if i in random_list], axis=0
        )
        # we compute the average experimental frequencies for the remaining ("B") subset
        matrix_mean_test = np.mean(
            [x for i, x in enumerate(frequencies_list) if i not in random_list], axis=0
        )
    ...
    while np.abs(old_delta - new_delta) > nu_delta:
        ...
        list_effects = temp_var[1]
        # print(new_delta)
        if new_delta < av_distance / 2.0 or np.abs(old_delta - new_delta) < nu_delta:
    ...
    for jj in range(0, num_outcomes):
        ...
        for kk in range(0, num_inputStates):
            logF += (
                (-1)
                * frequencies[jj, kk]
                * cp.log(cp.real(cp.trace(Pi_list[jj] @ list_states[kk])))
            )
...
def anRes(
    frequencies_matrix, list_states, which_method="SDP", SDPtype="norm", **kwargs
):
    """
    ...
    if which_method == "SDP":
        ...
        vector_effects = run_SDP_QMT(
            list_states=list_states,
            frequencies=frequencies_matrix,
            SDPtype=SDPtype,
            **kwargs
        )[1]
        ...
        starttime = timeit.default_timer()
        vector_effects = run_MLE_log(
            list_states=list_states, frequencies=frequencies_matrix
        )
        stoptime = timeit.default_timer()
...
def random_experiment_methods(dim, num_shots, num_states, num_exps, **kwargs):
    ...
    for keyMethod in ["singleDelta", "norm", "logMLE"]:
        tracedist_matrix_Dict[keyMethod] = np.zeros(
            (num_exps, dim**2)
        )  # we store here the trace distances
    ...
    for ll in range(num_exps):
        ...
        while (
            np.linalg.matrix_rank(
                np.column_stack(
                    [
                        np.ndarray.flatten(random_states[ll, :, :])
                        for ll in range(num_states)
                    ]
                )
            )
            < dim**2
        ):  # we want a set of input states that forms a basis in the space of system operators
            for i in range(0, num_states):
                random_states[i] = rand_dm_ginibre(
                    dim
                )  # we generate random input states
        ...
        frequencies_matrix = generate_frequencies(
            num_shots=num_shots, list_effects=listEffects, list_states=random_states
        )
        # we simulate a matrix of experimental outcome frequencies

        # here we apply the different fitting methods
        [time_vector_Dict["logMLE"][ll], vector_effects_MLE] = anRes(
            frequencies_matrix=frequencies_matrix,
            list_states=random_states,
            which_method="logMLE",
            SDPtype="singleDelta",
        )
        [time_vector_Dict["singleDelta"][ll], vector_effects_sD] = anRes(
            frequencies_matrix=frequencies_matrix,
            list_states=random_states,
            which_method="SDP",
            SDPtype="singleDelta",
        )
        [time_vector_Dict["norm"][ll], vector_effects_norm] = anRes(
            frequencies_matrix=frequencies_matrix,
            list_states=random_states,
            which_method="SDP",
            SDPtype="norm",
        ...
        for jj in range(dim**2):
            ...
            tracedist_matrix_Dict["logMLE"][ll, jj] = trace_distance(
                vector_effects_MLE[jj], listEffects[jj]
            )
            tracedist_matrix_Dict["singleDelta"][ll, jj] = trace_distance(
                vector_effects_sD[jj], listEffects[jj]
            )
            tracedist_matrix_Dict["norm"][ll, jj] = trace_distance(
                vector_effects_norm[jj], listEffects[jj]
            )

        for keyMethod in ["singleDelta", "norm", "logMLE"]:
            resDict[keyMethod] = {}
            resDict[keyMethod]["vecMean"] = np.mean(
                np.mean(tracedist_matrix_Dict[keyMethod], axis=1)
            )  # average trace distance
            resDict[keyMethod]["vecStd"] = np.std(
                np.mean(tracedist_matrix_Dict[keyMethod], axis=1)
            )
            resDict[keyMethod]["timeMean"] = np.mean(
                time_vector_Dict[keyMethod]
            )  # average runtime
