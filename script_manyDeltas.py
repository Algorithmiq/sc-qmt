...
vec_manydeltas_Pauli_incoh = np.zeros((4,6,num_exps)) # We store here the values of the deltas for incoherent noise
vec_manydeltas_Pauli_coh = np.zeros((4,6,num_exps)) # We store here the values of the deltas for coherent noise

zeroNoise = np.zeros(6)
cohNoiseVec = [0.0,0.01,0.0,0.0,0.0,0.0] # vector for the coherent noise on the second input state
incohNoiseVec = [0.0,0.00,0.1,0.1,0.0,0.0] # vector for incoherent noise on the third and fourth input state

for jj in range(num_exps): # loop over num_exps experiments
    # We generate the experimental frequencies with noisy input states    
    noisy_freqPaulis_incoh = generate_frequencies_noisy_states_vec(num_shots,M,rhoPaulis,incohNoiseVec,zeroNoise)
    noisy_freqPaulis_coh = generate_frequencies_noisy_states_vec(num_shots,M,rhoPaulis,zeroNoise,cohNoiseVec)
    
    # We process the data through the many-deltas SDP
    vec_manydeltas_Pauli_incoh[:,:,jj] = run_SDP_manyDeltas(rhoPaulis,noisy_freqPaulis_incoh)[0]
    vec_manydeltas_Pauli_coh[:,:,jj] = run_SDP_manyDeltas(rhoPaulis,noisy_freqPaulis_coh)[0]
