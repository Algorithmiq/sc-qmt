...
vec_delta_ideal4 = np.zeros((num_sets,num_exps_per_set,2)) # vector of final deltas and numbers of seesaw steps (4 random input states, only shot noise)
vec_delta_noisy4st001 = np.zeros((num_sets,num_exps_per_set,2)) # vector of final deltas and numbers of seesaw steps (4 random input states, coherent noise)
...
for jj in range(num_sets):
    ...
    for kk in range(num_exps_per_set):   
        ...
        noisy_freq4001 = gen_frequencies_list_noisy(15000,M,rhoRand4,0.0,0.01,num_k=10) # coherent noise with epsilon = 0.01
        
        temp = run_seesaw_crossV(rhoRand4,id_freq4,1e-6,num_sampling=5,num_repetitions = 10, SDPtype = 'singleDelta')
        ...
        temp = run_seesaw_crossV(rhoRand4,noisy_freq4001,1e-6,num_sampling=5,num_repetitions = 10, SDPtype = 'singleDelta')
