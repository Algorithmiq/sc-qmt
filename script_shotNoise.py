...
for jj in range(num_exps):
    ...
    while np.linalg.matrix_rank(np.column_stack([np.ndarray.flatten(rhoRand[ll,:,:]) for ll in range(4)]))<4:
    ...
    vec_delta_100000[jj] = run_SDP_QMT(rhoRand,id_freq_100000,SDPtype = 'singleDelta')[0]
    vec_delta_5000000[jj] = run_SDP_QMT(rhoRand,id_freq_5000000,SDPtype = 'singleDelta')[0]
